function speak(text) {
 ...
 function askAI() {
  ...
  try {
    ...
    // Wait a moment before speaking the real answer
    setTimeout(() => {
      if (data.reply) {
        speak(data.reply); // 🗣️ Joey actually speaks the answer!
        alert("Joey says:\n\n" + data.reply);
      } else {
        alert("Joey had trouble: " + (data.error || "No response"));
      }
    }, 500); // 500ms gives the browser time to re-enable sound
  } catch (err) {
